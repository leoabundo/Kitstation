...
const FIELD_LIMITS = {
  ...
  mensaje: 5e3,
  detalle_reclamo: 5e3,
  descripcion_pedido: 5e3,
  descripcion_monto: 3e3
};
...
function cleanValue(value, limit = 500) {
  ...
  return value.trim().replace(/<[^>]*>/g, "").replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g, "").slice(0, limit);
}
...
const GET = handleMethodNotAllowed;
const PUT = handleMethodNotAllowed;
const PATCH = handleMethodNotAllowed;
const DELETE = handleMethodNotAllowed;
...
const POST = async ({ request, clientAddress }) => {
  try {
    ...
    const email = data.email || data.correo || data.correo_electronico || "";
    ...
    const excluded = /* @__PURE__ */ new Set(["formulario", "pagina_origen"]);
    const fieldRows = Object.entries(data).filter(([key]) => !excluded.has(key)).map(
      ([key, value]) => `<tr><th style="padding:8px 12px;text-align:left;vertical-align:top;background:#f5f5f5;">${htmlValue(fieldLabel(key))}</th><td style="padding:8px 12px;">${htmlValue(value)}</td></tr>`
    ).join("");
    const plainFields = Object.entries(data).filter(([key]) => !excluded.has(key)).map(([key, value]) => `${fieldLabel(key)}: ${value}`).join("\n");
